...
@section('body')
    ...
    <div class="bg-gradient-pine h-screen">
        @include('_layouts.default.partials.headers.master', ['color' => 'green'])

        <div class="container flex h-screen items-center">
            <div class="mx-auto text-center content">
                <a href="https://github.com/ThemeMountain/pine/releases" target="_blank" rel="noopener" class="bg-transparent hover:bg-white text-sm text-white font-semibold hover:text-{{ $page->color }} py-1 px-2 border border-white rounded no-underline">
                  v{{ $page->version }}
                </a>
